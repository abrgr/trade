...
(defn mkt-ids-for-strat [venue-state strat-id]
  (->> venue-state
       :req-order-books
       strat-id
       (map :market-id)
...
(defn contract-ids-for-strat [venue-state strat-id]
  (->> (mkt-ids-for-strat venue-state strat-id)
       (map #(get-in venue-state [:contracts %]))
       (mapcat keys)))
...
(defn strat-bankroll [venue-state cfg strat-id venue-id]
  (let [bal (:bal venue-state)
        ...
        mkts (mkt-ids-for-strat venue-state strat-id)
...
(defn get-orders [state venue-id venue-state mkt-id]
  (let [venue (->> state
        ...
        orders-by-contract-id (->> contract-ids
                                   ...
                                   (into {}))]
    (send (:venue-state state) #(assoc-in % [:orders mkt-id] orders-by-contract-id))
...
(defn outstanding-orders-for-strat [state venue-id venue-state strat-id]
  (let [{:keys [outstanding-orders]} venue-state
        mkt-ids (mkt-ids-for-strat venue-state strat-id)
...
(defn desired-pos-for-req-pos [bankroll req-pos]
  {:contract-id (:contract-id req-pos)
   ...
   :price (:price req-pos)
   :shares (Math/floor (/ (* (:desired bankroll) (:weight req-pos)) (:price req-pos)))})
                 ...
                 (let [desired-side-sell (-> desired-side sell-trade-type-for-side)
                     ...
                     (when-not (empty? ords-to-cancel)
                       (map
                          ...
                          ords-to-cancel)))
                 ; we want to buy
                 (let [desired-price (:price desired-pos)
                       still-valid-order? #(< (Math/abs (- (:price %) desired-price)) 0.05)
                       cur-ords (-> desired-pos
                                    :trade-type
                                    ords-by-trade-type)
                       ords (->> cur-ords
                       ...
                       remaining-qty (min max-qty (- (:qty desired-pos) cur-qty ord-qty))]
                    (concat
                      ; our buy order
...
(defn execute-trades [state end-chan venue-id req-pos-by-strat-id]
  (l/with-log :info "Executing trades"
    (let [venue-state @(:venue-state state)
          {:keys [cfg]} state]
      (doseq [[strat-id req-positions] req-pos-by-strat-id]
